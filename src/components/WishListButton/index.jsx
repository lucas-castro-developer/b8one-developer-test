...
function WishListButton(props) {
  const [addedToWishLishProd1, setAddedWishLishProd1] = useState(false);
  const [addedToWishLishProd2, setAddedWishLishProd2] = useState(false);

  const manageProductToWishLish = (id) => {
    if (id === 1) {
      addedToWishLishProd1
        ? setAddedWishLishProd1(false)
        : setAddedWishLishProd1(true);
    } else if (id === 2) {
      addedToWishLishProd2
        ? setAddedWishLishProd2(false)
        : setAddedWishLishProd2(true);
    } else {
      console.error(
  ...
  return (
    <button
      ...
      onClick={() => manageProductToWishLish(props.productId)}
    >
      <svg
        width="48"
        height="48"
      ...
      >
        <circle
          ...
          fill="#F2F3F6"
          className={
            addedToWishLishProd1 || addedToWishLishProd2
              ? "wish-list-added-circle"
              : "wish-list-default-circle"
          }
        />
        <path
          ...
          className={
            addedToWishLishProd1 || addedToWishLishProd2
              ? "wish-list-added-path"
              : "wish-list-default-path"
          }
        />
      </svg>
    </button>
